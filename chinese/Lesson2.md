...
有其他的方式做到不过拟合。通常这被叫做正则化（regularization）。正则化和所有这些技术保证我们的模型不仅仅能在它看到的数据上工作得很好，也可以在它没有看到的数据上工作得很好。你训练模型是最重要的事情，是知道，他在他没有看到过的数据上工作得怎样。就像我们会在下周学到的那样，这就是为什么我们有验证集的原因。
